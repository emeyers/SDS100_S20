...
The purpose of this homework is to practice running parametric hypothesis tests and computing parametric confidence intervals for one and two means. Please submit a compiled pdf with your answers to the exercises to Gradescope *by 11:30pm on Sunday April 12th*.  
...
**Exercise 1.1 (5 points)**: Find the endpoints (quantile function values) of a t-distribution with 5% beyond them in each tail if the sample has a size of n = 10. Assume that the underlying distribution of the data is relatively normal (so n = 10 is a large enough sample size to use normal/t-based distributions). Hint: all quantile functions start with the letter *q*, and then have the distribution name, so for quantiles of the t distribution we use the qt() function and we specify the area we want and the degrees of freedom (df), i.e., qt(area, df = degrees_of_freedom). 
...
**Exercise 3.3 (13 points)**: One other method we have used to test whether two means are different is resampling methods. Let's again treat the two samples as if they were independent and run a resampling test to assess whether there are differences in the quiz vs. lecture pulse rates by doing the following steps:

1. compute the observed statistic

2. create a null distribution: i.e., combine the data, repeat 10,000 times: shuffle the combined data, separate into groups, create null statistic.
...
Please reflect on how the homework went by going to Canvas, going to the Quizzes link, and clicking on [Reflection on homework 9](https://yale.instructure.com/courses/55573/quizzes/23974).
