...
In the following homework you will gain practice running hypothesis tests for more than two means and corrleations. Please submit a compiled pdf with your answers to the exercises to Gradescope *by 11:30pm on Sunday March 29th*.  

A list of functions we have used in class can be found on [Canvas](https://yale.instructure.com/courses/55573/pages/r-functions-used-in-the-class). You might also find the following symbols useful:  $\mu$, $H_0$, $H_a$, $\rho$,  $\ne$, and $\alpha$. For full credit on problems involving R, be sure to label all your figures and to "show your work" by making sure all values that are answers to questions are printed/visible in your R Markdown pdf.


As always, if you need help with any of the homework assignments, please attend my online office hours or the TA online office hours which are listed on Canvas. Additionally, please ask any questions you have on [Piazza](https://piazza.com/class/k52xi66z6yv5ns). Also, if you have completed the homework, please help others out by answering questions on Piazza. Finally, if you have conceptual questions, reviewing the class slides and looking at the Lock5 textbook will be helpful. 
...
```{r message=FALSE, warning=FALSE, tidy=TRUE, echo = FALSE}
    ...
    # download_class_data("movies.Rda")
    # download_class_data('draft_lottery_data.Rda')
    # download_image('Lock5_4_26.png')
    # download_image('1969_draft_lottery_photo.jpg')
...
**Exercise 1.1 (6 points)**:  Let's now start our hypothesis test with step 1 by stating the null and alternative hypotheses in symbols and words. Also state the alpha level that is most commonly used. 
...
**Exercise 1.2a (6 points)** Let's now visualize the data by creating a side-by-side boxplot comparing the critics' scores of the movie for each MPAA rating level. Does it appear that the critics' scores differ on average depending on the MPAA classification of the movie? 
...
**Part 1.2b (5 points)**:  Next let's use the MAD statistic that we discussed in class to compare the mean critic scores between the different MPAA rating levels. To do this we can use the `get_MAD_stat(quantitative_data, grouping_data)` function from the ClassTools library. Save the MAD statistic value to an object called `obs_stat` and report the value of this statistic below.  
...
**Exercise 1.4 (5 points)**: Let's do step 4 of the hypothesis test by calculating the p-value using the ClassTools `pnull` function. Report what the p-value is. Is this close to what you estimated by looking at the null distribution above? 
...
**Exercise 1.5 (5 points)**:  Now complete step 5 of hypothesis testing by making a judgement. Are you able to reject the null hypothesis? What do you conclude? 
...
The exercise in part 2 is based on a question from the Lock5 textbook. For addtional practice problems please see the textbook. 
...
**Exercise 2.1: Mercury Levels in Fish (15 points)**  Figure 4.26 shows a scatterplot of the acidity (pH) for a sample of n = 53 Florida lakes vs the average mercury level (ppm) found in fish taken from each lake (the full data set, called FloridaLakes, can be obtained from the Lock5 website at:  http://www.lock5stat.com/datapage.html). There appears to be a negative trend in the scatterplot, and we wish to test whether there is significant evidence of a negative association between pH and mercury levels. 
...
![Figure 4.26](/home/shared/intro_stats/images/Lock5_4_26.png)
...
![Draft Lottery Image](/home/shared/intro_stats/images/drawing.jpg)
...
**Exercise 3.1 (5 points)**: Let's start by doing step 1 of our null hypothesis significance tests (NHSTs) by stating the null and alternative hypotheses in symbols and in words, and state the significance level.
...
**Part 3.2b (5 points)**: Next let's do step 2 of hypothesis testing by calculating the statistic of interest and saving it to an object called `obs_stat`. Describe what this statistic means as clearly as you can (e.g., if the statistic is negative what does that mean in terms of dates and draft numbers?). 
```{r message=FALSE, warning=FALSE, tidy=TRUE}
...
**Part 3.3 (10 points)**: Now let's do step 3 of hypothesis testing by creating a null distribution. You can calculate one point in the null distribution by shuffling the draft_numbers (using the ClassTools `shuffle()` function) and then calculating the correlation. Use the `do_it()` to repeat this process 10,000 times to generate the full null distribution. Plot a histogram of the null distribution, put a red vertical line on it at the value of the observed statistic, and describe the null distribution's shape. Is the center of the null distribution at a value that makes sense to you? Also, report what you expect the p-value to be. 
...
**Part 3.5 (5 points)**: Make a judgement call as to whether you believe the draft lottery was fair. Make sure to justify your answer.  
...
**Exercise 4.1:  (21 points)**  The code below loads data from this study. In particular, it creates an object called `uniform_malevolence` which has the "malevolence index" for each uniform, and an object called ` z_penalty_mins` which has the z-score of the number of penalty minutes each team had. 
...
Please reflect on how the homework went by going to Canvas, going to the Quizzes link, and clicking on [Reflection on homework 7](https://yale.instructure.com/courses/55573/quizzes/23392).
